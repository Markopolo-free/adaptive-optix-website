...
export default function BaaSUseCasePage() {
  const [data, setData] = useState<any>(null);

  useEffect(() => {
    async function fetchData() {
      const res = await fetch('/api/use-case-page?slug=baas');
  ...
  return (
    <main style={{ backgroundColor: '#000029ff', color: '#fff', minHeight: '100vh', padding: '60px 0', position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'row', alignItems: 'flex-start' }}>
      ...
      <div className="flex flex-col items-start" style={{ flex: 1, maxWidth: '900px' }}>
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-white mb-10" style={{ marginBottom: '40px' }}>{data?.title || 'Banking as a Service'}</h1>
        <div className="text-xl text-white bg-[#14143A] border-t-4 border-blue-500 rounded-2xl p-8 sm:p-12 lg:p-16 mb-14 w-full" style={{ boxSizing: 'border-box', wordBreak: 'break-word', marginBottom: '48px' }}>
          {data?.body ? (
            ...
            <>
              <p className="mb-6">As a BaaS operator, it is crucial to offer customer value that is a differentiator.</p>
              <p className="mb-6">Often overlooked or over-simplified by many EMI or Fintech operators, effective BaaS-based pricing and value management can enrich customers’ experiences.</p>
              <p className="mb-6">Leveraging fundamental pricing principals and combining these with more sophisticated approaches - depending on customers and their behaviors - organizations can significantly increase the value offered to customers and in turn revenue.</p>
              <p>Our solution allows you to easily integrate and offer a feature-rich, reliable and scalable service that enables customers to develop, deploy and monetize their competitive pricing strategies effectively.</p>
            </>
          )}
        </div>
        <div className="bg-[#14143A] border-t-4 border-blue-500 rounded-2xl p-8 flex flex-col items-center w-full" style={{ marginTop: '0', marginBottom: '0' }}>
          {imageUrl ? (
            <Image
              ...
              alt={data?.title || 'Banking as a Service Diagram'}
              width={850}
              height={850}
